...
def payment(request):
        ...
        from_email=None, # use DEFAULT_FROM_EMAIL if None
        recipient_list=[i[2]],)
                            h=journeys(id1=p[0][7],journey=w[1],price=t,date=p[0][6],time="15:00:00",user=request.user.username)
                            h.save()
...
def journeys1(request):
 ...
 # Example database query
    ...
    # Provide alternative actions or informative messages
...
def edit_passenger(request,passenger_id):
    ...
    if request.method == 'POST':
        # Retrieve and validate data manually
        name = request.POST.get('name')
        email = request.POST.get('email')  # Add other fields as needed
        ...
        passenger.name = name
        passenger.email = email  # Update other fields as needed
        passenger.save()

        return redirect('custview')  # Redirect to the appropriate view
